  ...
  <div class="goods" id="goods" name="goods">
    ...
    <table>
      ...
      <tr v-for="(item, index) in items" :key="index">
        ...
        <td>{{ item.price }}</td>
        <td>{{ item.category.level1 }}</td>
        <td>{{ item.category.level2 }}</td>
        <td><button @click="editItem(index)">修改</button></td>
    ...
    <form @submit.prevent="addProduct">
      ...
      <br>
      <label>主图:</label>
      <input type="text" v-model="newProduct.imageUrl" required>
      <br>
      <label>商品价格:</label>
      <input type="number" v-model="newProduct.price" required>
      <br>
      <label>一级分类:</label>
      <input type="text" v-model="newProduct.category.level1" required>
      <br>
      <label>二级分类:</label>
      <input type="text" v-model="newProduct.category.level2" required>
      <br>
    ...
    <div class="main">
      <div class="list">
        <MyList :list="goods" v-if="goods.length>0"></MyList>
...
activated() {
  ...
  methods: {
    ...
    deleteItem(index) {
      this.$axios
          .get("/api/cate/deleteGoods?id=" + index.id)
          .then(res => {
            const val = {
              categoryId: 0,
              categoryName: "全部"
      ...
      editItem(index) {
        this.$axios
            .post("/api/cate/updateGoods",index)
            .then(res => {
              const val = {
                categoryId: 0,
                categoryName: "全部"
    ...
    addProduct() {
      this.$axios
          .post("/api/cate/addGoods",this.newProduct)
          .then(res => {
            this.goodsList = res;
          });
    ...
    getData() {
      ...
      //   });
      let api = "/api/goods/page/" + this.currentPage + "/" + this.pageSize;

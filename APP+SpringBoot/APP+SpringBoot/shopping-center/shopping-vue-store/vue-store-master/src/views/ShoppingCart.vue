  ...
  <div class="shoppingCart">
    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <!-- 购物车列表 -->
        <li class="goods-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          <div class="pro-check">
          ...
          <div class="pro-img">
            <router-link :to="{ path: '/goods/details', query: {goodsID:item.goodsId} }">
              <img :src="$target + item.goodsImg" />
            </router-link>
          </div>
          <div class="pro-name">
            <router-link
              :to="{ path: '/goods/details', query: {goodsID:item.goodsId} }"
...
.shoppingCart .content ul .goods-list {
  height: 85px;

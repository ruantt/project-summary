  ...
  <div id="myList" class="myList">
    <ul>
      <li v-for="item in list" :key="item.goodsId">
        ...
        <router-link :to="{ path: '/goods/seckillDetails', query: {seckillID:item.seckillId,goodsID:item.goodsId} }">
          <img :src="$target +item.goodsPicture" alt />
          <h2>{{item.goodsName}}</h2>
          <h3>{{item.goodsTitle}}</h3>
          <p>
            <span>{{item.seckillPrice}}元</span>
            <span
              v-show="item.goodsPrice != item.goodsSellingPrice"
              class="del"
...
export default {
  ...
  methods: {
    deleteCollect(goods_id) {
      this.$axios
        .delete("/api/collect/user/" + goods_id + "/" + this.$store.getters.getUser.userId)
        .then(res => {
